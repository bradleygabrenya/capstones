<template>
  <div>
      <button type="button" class="btn btn-primary btn-lg" v-show="this.$store.state.user.checkedIn.toUpperCase() == 'FALSE' " v-on:click="startWorkout">Start Workout</button>
      <button type="button" class="btn btn-primary btn-lg" v-show="this.$store.state.user.checkedIn.toUpperCase() == 'TRUE'" v-on:click="continueWorkout">Continue Workout</button>
  </div>
</template>
